...
use pyo3::{Py, pyclass, pymethods, PyRef, PyRefMut, PyResult, Python};
...
pub struct Graph {
    #[pyo3(get, set)]
    pub nodes: Vec<GraphNodeRS>,
    names_to_index: Option<HashMap<String, i32>>,
    need_update: bool,
}
...
#[pymethods]
impl Graph {
    fn __getitem__<'py>(&self, key: usize) -> PyResult<&'py GraphNodeRS> {
        Ok(self.nodes.get(key)?)
    }

    fn delete_node(&mut self, key: usize) {
        self.nodes.remove(key);

        for node in self.nodes.iter_mut() {
            let mut edges_to_del = Vec::new();
            let mut edges_to_decrease = Vec::new();
            for (index, edge_tail) in node.edges.iter().enumerate() {
                if edge_tail.next_node_index == key {
                    edges_to_del.push(index)
                } else if edge_tail.next_node_index > key {
            ...
            for index in edges_to_decrease.iter() {
                node.edges.get(index)?.next_node_index -= 1
            }
            edges_to_del.sort();
    ...
    #[getter]
    fn edges(&self, py: Python) -> PyResult<Py<EdgesIterator>> {
        let rs_iter = self.nodes
            .iter()
            .map(|node| node.edges.iter())
            .flatten();
        let py_iter = EdgesIterator {
            iter: rs_iter
        };
        Py::new(py, py_iter)
    }

    fn copy_graph(&self) -> Self {
        self.clone()
    }

    fn delete_nodes(&mut self, mut node_indexes_to_del: Vec<usize>) {
